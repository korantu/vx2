...
void Scene::Init(bool changeLights)
{
	...
	if( lights==NULL && maxLights>0 )
	{
		...
		NumLights = 2;				 // default number of lights
...
void Scene::CreateBar()
{
	...
	TwDefine(" Lights label='Lights TweakBar' help='Use this bar to edit the lights in the scene.' ");

	// Add a variable of type int to control the number of lights
	TwAddVarRW(lightsBar, "NumLights", TW_TYPE_INT32, &NumLights, " label='Number of lights' keyIncr=l keyDecr=L help='Changes the number of lights in the scene.' ");
	...
	TwAddButton(lightsBar, "Reinit", ReinitCB, this, " label='Change lights' key=RETURN help='Random changes of lights parameters.' ");
	...
	TwStructMember lightMembers[] =	// array used to describe tweakable variables of the Light structure
	{
		...
		{ "Speed",	   TW_TYPE_FLOAT,   offsetof(Light, Speed0),    " readonly help='Light moving speed.' " } // Light::Speed is made read-only
	};
	...
	for(int i=0; i<maxLights; ++i)	// Add 'maxLights' variables of type lightType; unused lights variables (over NumLights) will hidden by Scene::Update( )
	{
		...
		_snprintf(def, 255, "group='Edit lights' label='Light #%d' help='Parameters of the light #%d' ", i+1, i+1);
...
int main() 
{
	...
	// Create a tweak bar called 'Main' and change its refresh rate and position
	TwBar *mainBar = TwNewBar("Main");
	TwDefine(" Main label='Main TweakBar' refresh=0.5 position='32 200' ");

	// Add some variables to the Main tweak bar
	TwAddVarRW(mainBar, "Wireframe", TW_TYPE_BOOLCPP, &scene.Wireframe, " group='Display' key=w help='Toggle wireframe display mode.' ");	// 'Wireframe' is put in the group 'Display' (which is then created)
	TwAddVarRW(mainBar, "BgTop", TW_TYPE_COLOR3F, &scene.BgColor1, " group='Background' help='Change the top background color.' ");			// 'BgTop' and 'BgBottom' are put in the group 'Background' (which is then created)
	TwAddVarRW(mainBar, "BgBottom", TW_TYPE_COLOR3F, &scene.BgColor0, " group='Background' help='Change the bottom background color.' ");
	TwDefine(" Main/Background group='Display' ");	// The group 'Background' of bar 'Main' is put in the group 'Display'
	TwAddVarCB(mainBar, "Subdiv", TW_TYPE_INT32, SetSubdivCB, GetSubdivCB, &scene, " group='Scene' label='Meshes subdivision' min=1 max=50 keyincr=s keyDecr=S help='Sudivide the meshes more or less (switch to wireframe to see the effect).' ");
	...
	TwAddVarRW(mainBar, "Rotation", rotationType, &scene.Rotation, " group='Scene' keyIncr=Backspace keyDecr=SHIFT+Backspace help='Stop or change the rotation mode.' ");
